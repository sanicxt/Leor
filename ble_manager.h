...
static String bleProcessPayload(const String &payload) {
  // Allow sending multiple commands at once separated by newlines or
  // semicolons.
  String lastResponse;
  ...
  while (start < work.length()) {
    ...
    String cmd =
        (end == -1) ? work.substring(start) : work.substring(start, end);
...
void sendBLEChunked(NimBLECharacteristic *pChar, const String &data) {
  ...
  // For simplicity, we just send raw chunks. The frontend should reassemble or
  // handle partials.
  while (sent < total) {
    ...
    // In NimBLE, we can check if the queue is full, but a 5ms delay is usually
    // safe.
    if (sent < total)
      delay(5);
  }
...
String getBLEStatusInfo() {
  ...
  if (deviceConnected) {
    uint32_t connectedSecs =
        (millis() - (lastActivityMs > 0 ? lastActivityMs : millis())) / 1000;
...
void setBLELowPowerMode(bool enabled) {
  ...
  if (pAdv) {
    pAdv->stop();
    if (enabled) {
      pAdv->setMinInterval(BLE_ADV_INTERVAL_SLOW *
                           1.6); // Convert ms to 0.625ms units
...
bool getBLELowPowerMode() { return bleLowPowerMode; }
...
void initBLE(const char *deviceName) {
  ...
  pGestureChar = pService->createCharacteristic(
      GESTURE_CHAR_UUID, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::NOTIFY);
  if (pGestureChar) {
  ...
  // Always start at full TX power so the device is discoverable.
  NimBLEDevice::setPower(BLE_TX_POWER_HIGH);

  NimBLEAdvertising *pAdvertising = NimBLEDevice::getAdvertising();
  
  // Use NimBLE's built-in convenience methods. 
  // IMPORTANT: Only add ONE 128-bit UUID to avoid overflowing the 31-byte primary packet.
  // The device name goes into the primary packet automatically via setName().
  // NimBLE handles the packet layout internally.
...
void handleBLEConnection() {
  ...
  if (!deviceConnected && advRestartAtMs != 0 &&
      (int32_t)(now - advRestartAtMs) >= 0) {
    advRestartAtMs = 0;
    if (NimBLEDevice::getAdvertising()->start()) {
      advStartedAtMs = now;
