...
#define BLE_NOTIFY_MIN_INTERVAL_MS                                             \
  20 // Minimum ms between notifications (prevent flooding)
#define BLE_CONNECTION_TIMEOUT_MS                                              \
  30000 // Detect stale connections (30 seconds no activity)
#define BLE_MTU_REQUEST                                                        \
  185 // Request larger MTU (ESP32 max is typically 517, but 185 is safer)
#define BLE_ADV_WATCHDOG_MS                                                    \
  60000 // Restart advertising if no connection after 60s

// BLE Power Saving Settings
#define BLE_TX_POWER_LOW                                                       \
  ESP_PWR_LVL_N12 // -12dBm (ultra low power, ~2-5m range)
#define BLE_TX_POWER_HIGH ESP_PWR_LVL_P9 // +9dBm (high power, ~30m range)
#define BLE_ADV_INTERVAL_FAST                                                  \
  100 // Fast advertising interval (ms) - for quick connection
#define BLE_ADV_INTERVAL_SLOW                                                  \
  2000 // Ultra-slow advertising interval (ms) - extreme power saving

// Note: BLE uses significantly less power than WiFi
// Control via Bluetooth using a mobile app or web Bluetooth API

// ==================== Display Settings ====================
// Display type is selected at runtime via preferences (disp_type: sh1106 or
// ssd1306) Change via command: display:type=sh1106 or display:type=ssd1306
// (requires restart)
...
#define GESTURE_COOLDOWN_MS                                                    \
  1500 // Cooldown after gesture match before next detection
...
#define TOUCH_HOLD_DEFAULT_MS                                                  \
  3000 // Long-touch hold duration (max 3s via WebUI)
